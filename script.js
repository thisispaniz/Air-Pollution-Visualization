...
d3.json('2_hoch.geo.json').then(geojson => {
    ...
    function renderMap(geoJson, airQualityData) {
        // Assuming 'european_aqi' is the key in your air quality data
        const minAqi = d3.min(airQualityData, d => d.european_aqi);
        const maxAqi = d3.max(airQualityData, d => d.european_aqi);

        const colorScale = d3.scaleSequential(d3.interpolateOrRd)
            .domain([minAqi, maxAqi]);
            console.log('something', airQualityValue)
            const getColor = d => {
                if (d.airQualityValue && d.airQualityValue.current) {
                    return colorScale(d.airQualityValue.current.european_aqi);
                } else {
                    return 'black'; // or any default color for cases where data is not available
                }
            };
        ...
        const joinedData = geoJson.features.map(feature => {
            ...
            return { ...feature, airQualityValue: airQualityDatum ? airQualityDatum : null };
        });
